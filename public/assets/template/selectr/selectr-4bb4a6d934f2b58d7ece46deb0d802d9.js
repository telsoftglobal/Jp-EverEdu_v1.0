      ...
      Selectr.prototype.PrepareOpts = function(opts) {
        ...
        for (_i = 0, _len = opts.length; _i < _len; _i++) {
          ...
          _results.push($(document.createElement('li')).attr({
            'class': "list-group-item " + (opt.selected ? 'selected' : ''),id:opt.id
          }).data('val', $(opt).val()).append($(document.createElement('div')).attr({
            'class': "color-code fa fa-ticket " + (!$(opt).data('selectr-color') ? 'no-color' : void 0)
          ...
          })).append($(document.createElement('input')).attr({'type':'hidden','name':"curriculum[curriculum_categories_attributes]["+opt.id+"][category_id]",'value':opt.id}))
                  .append($(document.createElement('input')).attr({'type':'hidden','name':"curriculum[curriculum_categories_attributes]["+opt.id+"][category_name]",'value':$(opt).text()}))
          );
        }
        return _results;
      };
...
//            CuongCT edit 13-11-2014
           $("#panellevel .row").hide();
//           alert("#level"+$(this).attr('id'));
           var idshow=$(this).attr('id');

//            alert($("#level"+$(this).attr('id')).length);
           if($("#panellevel"+idshow).length==0) {
                //object already exists
               addCurriculumCategory(idshow,$(this).find("div.option-name").text());
           }

            $("#panellevel"+idshow).show();
            $("#categoryheader").text($(this).find(".option-name").text());
...
//#CuongCT;
//console.log($(this).parent("li").find("div.option-name").text());
            var textsearch =$(this).parent("li").find("div.option-name").text();

            //redisplay category
            $("ul.dropdown-menu").find("li:contains("+textsearch+")").show()
            $("#panellevel"+$(this).parent("li").attr("id")).remove();

            $(this).parent("li").remove();

            $("#categoryheader").text("");

            $("#categoryheader").siblings("div").hide();

...
FixedHeader.prototype = {
	...
	_fnCloneThead: function ( oCache )
	{
		...
		if (jQuery(s.nTable).parents('.dataTables_scroll').length)
		{
			var scrollHead = jQuery(s.nTable).parents('.dataTables_scroll').first().find('.dataTables_scrollHead');
			nThead = jQuery('thead', scrollHead).clone(true)[0];
			scrollHead.height(0);
			uTable = scrollHead.parent();
		}
		else
		{
			nThead = jQuery('thead', s.nTable).clone(true)[0];
			uTable = s.nTable;
		}
		...
		jQuery("thead>tr th", uTable).each( function (i) {
			jQuery("thead>tr th:eq("+i+")", nTable).width( jQuery(this).width() );
		} );
		
		jQuery("thead>tr td", uTable).each( function (i) {
